...
double Algorithm::predictDate(tm *Date, Stock*st) {
    cout << "predicDate in algorithm" << endl;  
    ...
    //This is the index value into the data list for the starting date   
    int indexVal = getDateIndex(Date, st, fund) - test Interval;   
    ...
    for (it = dates->begin() + indexVal; i < testInterval + 1; it++) {
        ...
        //Equality constraint  
        glp-set_row_bnds(lp, i, GLP_FX, actual, actual);   
        i++;  
    }


    // Number variables in factor size + numConstraint*2 (2 errors for each constraint)   
    int numVars = (int) (faName->size() + testInterval * 2);   


    //Creates the variables 
    glp_add-cols(lp, numVars);  
    ...
    for (itr = faName->begin(); itr != faName->end(); itr++) {
        ...
        for (it = dates->begin() + indexVal - 1; i < testInterval + 1; it++) {
            ...
            js[j] = varNum; //Loops through variables   
    ...
    for (int n = 1; n < testInterval + 1; n++) {
        //Loop through all possible erro values  
    ...
    glp_load-matrix(lp, size - 1, ia, ja, ar);   
    ...
    for (int h = 1; h < (int) faName->size() + 1; h++) {
        ...
        results[h] = glp_get-col_prim(lp, h);   
    }  
...
double* Algorithm::selectStockDistribution(map<string, double> *percentCorrect, map<string, double> *increase, tm *prevDate) {  
    ...
    //eq7(i)..        Y(i) - a(i) =L= 0;                                NumSt eqs            //Can't buy a stock if it isn't supposed to increase    
    for (stock = stockList->begin(); stock != stockList->end(); stock++) {  
        glp_set_row_bnds(mip, i, GLP_UP, (increase->finf(stock->second.getName())->second),  
