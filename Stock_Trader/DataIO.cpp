...
DataIO::DataIO(string stockFile, string investForFile, string fundFile) {
 ...
 *
 */
void DataIO::initializeStockLocal() {

	cout << "Initializing Stocks\n";  

	extern char fileDelimDefault;  // Delimter in file  
...
void DataIO::initializeAllInvestorLocal() {
	...
	//Create a vector to hold the factorNames
	vector<string> vec;  
	...
	while (input.good()) {
		...
		if (i == 1) {
		    //Parses header and gets the name of the factors  
...
vector<tm*>* DataIO::getDates(string filename, char delim) {  
	cout << "Getting Dates." << endl;  	
	   ...
	   //Gets the first line of headers and makes date the object from them   
	   if (input.good()) {  
		...
		//Separates at the delimeter and inserts into the vector  
